...
desenhaJogo it = Pictures [picMapa,  
    ...
    where picMapa = desenhaMapa mapa texturas
          jogo = jogoIT it
          mapa = mapaJogo jogo
          moldBaixo = translate 0 (-520) $ fromJust $ lookup "moldBaixo" texturas 
          moldCima = fromJust $ lookup "molduraMapa2" texturas
          ...
          loja = lojaJogo jogo
          perfilJog = desenhaPerfilJogador it texturas 
...
desenhaLoja loja ts = Pictures [iconeLoja, 
    ...
    where x = -805
          ...
          fundoTorre1 =  Translate (-730) 100 $ scale 4 4 $ fromJust $ lookup "perfilJogador" ts
          fundoTorre2 = Translate (-730) (-100) $ scale 4 4 $ fromJust $ lookup "perfilJogador" ts
          fundoTorre3 = Translate (-730) (-300) $ scale 4 4 $ fromJust $ lookup "perfilJogador" ts
...
desenhaPerfilJogador it ts  
    ...
    | otherwise         = Pictures [perfilJogador, 
                            creditos, 
                            iconeVida, 
                            vidaBaseJg, 
                            perfilJogador, 
